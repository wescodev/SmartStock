...
{
    internal class UpdateProductQtdScreen
    {
        public static void Load()
        {
            Console.Clear();
            Console.WriteLine("Atualizar quantidade no estoque");
            Console.WriteLine("-------------");
            ...
            if (int.TryParse(Console.ReadLine(), out productId))
            {
                ...
                Atualizar(productId, quantity);
            }


        }

        private static void Atualizar(int idProduct, int quantity)
        {

            try
            {
                var repository = new ProductRepository(Database.Connection);
                repository.UpdateQtdStock(idProduct, quantity);
